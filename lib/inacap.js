...
const inacap = (jar = []) => {
  const urls = {
    ...
    asistencia: 'https://siga.inacap.cl/Inacap.Siga.AsistenciaAlumno/wServices/admProcesos.asmx/getAsignaturasAlumno'
  }
  ...
  const postForm = (url, data = {}, jar, prevRes, json = false) =>
  ...
  const getAttendance = period =>
    postForm(urls['asistencia'], {
      peri_ccod: period,
      sesi_ccod: sessionCode()
    }, cookies, null, true)
      .then(res => JSON.parse(res.text).d.data)
